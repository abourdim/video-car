#!/usr/bin/env bash
# ota_flash.sh — push firmware to the car over WiFi OTA
#
# Usage: ./ota_flash.sh <car-ip> [path/to/firmware.bin]
#
...
#
# ArduinoOTA runs whenever the car has WiFi up, whether that's its own
# "keyes1" AP (192.168.4.1) or a router it joined -- no special mode needed.
# See README.html's OTA section.
...
if [ -z "${1:-}" ]; then
    echo "Usage: $0 <car-ip> [path/to/firmware.bin]"
    echo
    echo "Default IP on the car's own AP (keyes1) is 192.168.4.1."
