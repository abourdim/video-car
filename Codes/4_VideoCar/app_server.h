...
// Joystick control: GET /joystick?x=<-100..100>&y=<-100..100>
// x: negative = left, positive = right. y: negative = back, positive = forward.
...
static esp_err_t joystick_handler(httpd_req_t *req) {
  ...
  if (jy > 100) jy = 100;
  else if (jy < -100) jy = -100;

  // Arcade mixing, matching the sign convention already used by the
  // Left/Right/Forward/Backward cases below (M1 = throttle-steer, M2 = throttle+steer).
  int s1 = jy - jx;
  int s2 = jy + jx;
  if (s1 > 100) s1 = 100;
  ...
  if (jx == 0 && jy == 0) {
    ...
    if (s1 >= 0) {
      i2c_Write(M1_PWM, pwm1);
      i2c_Write(M1_IN, 0);
    } else {
      i2c_Write(M1_PWM, 0);
      i2c_Write(M1_IN, pwm1);
    }
    if (s2 >= 0) {
      i2c_Write(M2_PWM, 0);
      i2c_Write(M2_IN, pwm2);
    } else {
      i2c_Write(M2_PWM, pwm2);
      i2c_Write(M2_IN, 0);
    }
    actstate = (jy >= 0) ? fwd : rev;
  }
